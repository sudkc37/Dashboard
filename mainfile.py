...
def get_food_data():
    ...
    query = """
        ...
        JOIN [Resturant].[dbo].[item] i ON o.item_id = i.item_id
...
def get_delivery_data():
    conn = get_db_connection()
    query = """
        select delivery_city, sum([Resturant].[dbo].[order].quantity * [Resturant].[dbo].[item].price) as total_revenue, COUNT([Resturant].[dbo].[order].add_id) AS delivery_count
        from [Resturant].[dbo].[order] 
        join [Resturant].[dbo].[item] on [Resturant].[dbo].[order].item_id = [Resturant].[dbo].[item].item_id
...
def get_average():
    conn = get_db_connection()
    query = """
        select avg([Resturant].[dbo].[order].quantity * [Resturant].[dbo].[item].price) as average_order_value
        from [Resturant].[dbo].[order]
        join [Resturant].[dbo].[item] on [Resturant].[dbo].[order].[item_id] = [Resturant].[dbo].[item].[item_id]
    """
    df = pd.read_sql(query, conn)
...
def get_totalRevenue():
    conn = get_db_connection()
    query = """
        select [Resturant].[dbo].[order].quantity * [Resturant].[dbo].[item].price as average_order_value
        from [Resturant].[dbo].[order]
        join [Resturant].[dbo].[item] on [Resturant].[dbo].[order].[item_id] = [Resturant].[dbo].[item].[item_id]
         """
    df = pd.read_sql(query, conn)
...
def get_sales_per_day():
    ...
    SUM([Resturant].[dbo].[order].quantity * [Resturant].[dbo].[item].price) AS sales
    FROM 
    [Resturant].[dbo].[order]
    JOIN [Resturant].[dbo].[item] ON [Resturant].[dbo].[order].[item_id] = [Resturant].[dbo].[item].[item_id]
